<template>
  <div>
    ...
    <div class="d-flex gap-2 py-3">
      <button type="button" class="btn btn-outline-primary" @click="openAddPaymentTypes()">
        Nouveau payment type
      </button>
    </div>
          ...
          <table id="admintable" class="table table-striped" style="width:100%">
            ...
            <tbody class="table-group-divider">
              <tr v-for="(acRow, index) in this.AcRows" :key="index">
                ...
                <td class="text-start">
                  <button type="button" class="bi bi-trash3 btn btn-outline-danger mx-1" @click="deletePaymentTypes(acRow.id)" v-b-tooltip.hover title="Delete the payment type!">
                  </button>
                </td>
              </tr>
            </tbody>
...
export default {
  ...
  methods: {
    ...
    openAddPaymentTypes() {
      this.$router.push({ name: "AddPaymentTypes" });
    },
    deletePaymentTypes(paytypesid) {
      try {
        this.success = "";
        this.warning = "";
        this.error = "";
        if (confirm('Are you sure, you want to delete this payment type. Payment Type Id: ' + paytypesid)) {
          const url = urlexp.getdeletePaymentTypesByIdUrl(paytypesid);
