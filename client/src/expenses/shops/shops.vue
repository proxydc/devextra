<template>
  <div>
    ...
    <div class="d-flex gap-2 py-3">
      <button type="button" class="btn btn-outline-primary" @click="openAddShop()">
        Nouveau shop
      </button>
    </div>
          ...
          <table id="admintable" class="table table-striped" style="width:100%">
            ...
            <tbody class="table-group-divider">
              <tr v-for="(acRow, index) in this.AcRows" :key="index">
                ...
                <td class="text-start">
                  <button type="button" class="bi bi-trash3 btn btn-outline-danger mx-1" @click="deleteShop(acRow.id)" v-b-tooltip.hover title="Delete the shop!">
                  </button>
                </td>
              </tr>
            </tbody>
  ...
  methods: {
    ...
    deleteShop(shopid) {
      try {
        ...
        this.warning = "";
        this.error = "";
        if (confirm('Are you sure, you want to delete this shop. Shop Id: ' + shopid)) {
          const url = urlexp.getdeleteShopByIdUrl(shopid);
